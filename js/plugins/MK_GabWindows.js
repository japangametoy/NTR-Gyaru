 ...
 * @target MZ
 * @author Aerosys
 * @plugindesc [Tier 1] [Version 1.0.6] [MV & MZ]
 * 
 * @help
 * 
 * ----------------------------------------------------------------------------
...
Gab_Window.prototype.drawTextEx = function(text, x, y, width, align) {
    if ('center' == align)  x += width / 2 - this.calculateLineWidth(text) / 2;
    if ('right' == align)   x += width - this.calculateLineWidth(text);
...
const alias_SceneMap_createAllWindows = Scene_Map.prototype.createAllWindows;
Scene_Map.prototype.createAllWindows = function() {
    alias_SceneMap_createAllWindows.call(this);
...
Scene_Map.prototype.update = function() {
    ...
    syncGabWindows(this, true);
}

const alias_SceneBattle_createAllWindows = Scene_Battle.prototype.createAllWindows;
Scene_Battle.prototype.createAllWindows = function() {
    alias_SceneBattle_createAllWindows.call(this);
...
function syncGabWindows(scene, showOpenAnimation) {
    ...
    Object.entries($gameScreen.gabWindows())
        ...
        .filter(([key, _]) => !scene._windowLayer.children.some(child =>
            child instanceof Gab_Window &&
            child._id == key
        ))
        .forEach(([key, value]) => registerGabWindow(scene, key, value, showOpenAnimation));
    ...
    scene._windowLayer.children
        ...
        .forEach(child => scene._windowLayer.removeChild(child));
}

function registerGabWindow(scene, id, data, showOpenAnimation) {
    const x = data.x || 0;
    const y = data.y || 0;
