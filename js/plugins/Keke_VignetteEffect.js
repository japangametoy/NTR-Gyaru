 ...
 *
 *
 * 【応用1】プラグインコマンドのパラメータを変数で指定
 * ◎プラグインコマンドのパラメータは全て変数を使うことができる
 * ◎書式は
 * \v[変数ID]
 * 例)
 * \v[10]
 ...
 *
 *
 * Vignette effect that generates and displays a hollow image
 * + Iris effect for smooth vignette changes
 *
 * ● Features ●
 *
 * Vignette effect
 * ◎ You can freely set the color, size, shape, etc.
 * ◎ Can be animated
 * ◎ A specific character can be tracked automatically.
 * ◎ Can be used without preparing an image
 * ◎ You can also use your favorite image
 * ◎ For both maps and battles
 *
 * ■Iris effect
 * ◎ Vignette shape can be changed smoothly
 * ◎ The screen can be closed and expanded in various ways
 * ◎ Can also be used as a transition
 *
 *
 * ●How to use ●
 *
 * [Basic 1] Display a vignette
 * => plugin command → vignette display
 * ◎ Generate and display an image that is hollowed out
 *   with a specific shape
 * ◎ Any number of vignettes can be displayed at the same time
 * ◎ Although it can be displayed without setting a name,
 *   Setting it makes it easier to specify the target vignette
 *   for iris effects, etc.
 * ◎ You can use your favorite image with "usePicture".
 *   If you don't use it, it becomes a plain image.
 *   In advance, plug-in parameters → picture registration
 *   It is necessary to register a picture
 * ◎ If "vignetteCarryOver" is set to true,
 *   it will not disappear even if the scene is moved.
 *   When erasing, use the plug-in command "DelVignette" manually
 *
 *
 * [Basic 2] Starting the iris effect
 * => Plugin command → Iris effect
 * ◎ Effect that smoothly changes the shape of the vignette
 * ◎ It is necessary to display the vignette in advance
 * ◎ Specify the target vignette and execute.
 *   Setting a vignette name makes it easier to specify
 * ◎ Enter only the items you want to change. Others can be left blank
 *   If the inner frame-radius X is set to 0, the radius X changes toward 0
 * ◎ Only one iris effect can be executed at a time.
 *   If multiple settings are made, they will be executed one by one
 * ◎ You can adjust the smoothness of the movement with "animeSheetsNum"
 *   The more sheets, the smoother but heavier. 15 sheets or less recommended
 *
 *
 * [Basic 3] Move, scale, and rotate the vignette
 * => plugin command → vignette operation
 * ◎ Vignette can be manipulated freely
 * ◎ It is necessary to display the vignette in advance
 * ◎ Specify the target vignette and execute.
 *   Setting a vignette name makes it easier to specify
 * ◎ Enter only the items you want to change. Others can be left blank
 *   If the shift X is set to 0, the shift X will change toward 0
 *
 *
 * [Basic 4] Change vignette settings
 * => plugin command → change vignette settings
 * ◎ Various vignette settings can be changed
 * ◎ It is necessary to display the vignette in advance
 * ◎ Specify the target vignette and execute.
 *   Setting a vignette name makes it easier to specify
 * ◎ Enter only the items you want to change. Others can be left blank
 *
 *
 * [Basic 5] Remove the vignette
 * => plugin command → remove vignette
 * ◎ Erase all or erase only the specified vignette
 *   Setting a vignette name makes it easier to specify
 *
 *
 * [Applied 1] Specify parameters of plug-in commands with variables
 * ◎Variables can be used for all parameters of plug-in commands
 * ◎The format is
 * \v[variable ID]
 * example)
 * \v[10]
 *   Contains the value of variable 10
 * 
 * 
 * [Applied 2] Using a vignette as a flashlight
 * [1] Plugin command → vignette creation
 * [2] Interior - form to "真円" (ellipse is also acceptable)
 * [3] Form Settings → Circular-Sectorize to ``60, 120''
 *   (example. Values ​​can be freely selected)
 * [4] Tracking settings → target character - name is "Player"
 *   "true" for rotation by orientation
 * 
 * 
 * [Applied 3] Vignette Common/Iris Common
 * ◎ Register the common in advance,
 * 　Vignette display and iris common can be easily executed
 * [1] Register common with plug-in parameter → common registration
 * [2] Run with plugin command → vignette common/iris common
 * ◎ When executing, write the name of the registered common
 *   in the "callCommon" field.
 * ◎ Common is a series of images like a flip animation.
 * 　Vignette common is to extract one of them
 * 　Iris common is played continuously
 * ◎ You can choose which position to acquire the image
 *   in the vignette common "acquisition point"
 * ◎ Iris common "startPoint" and "endPoint"
 *   You can set where to start and end playback.
 *   Reverse playback is also possible
 *
 *
 * [Supplement] Applied at the start
 * ◎ If the plug-in command "startingExe" is set to true,
 *   Automatically executed when the scene starts
 * ◎ There is no need to automatically execute the event
 *   Note that automatic execution will cause it to be executed twice
 * ◎ Must be placed on a valid page of the event
 * For battles, the first page of the battle event
 * ◎ The feature is that execution is faster than automatic execution
 * 　For the process that you want to apply immediately after starting
 *
 *
 * ● Terms of Use ●
 ...
 * 
 * @param ピクチャ登録
 * @desc pictureMaking ビネット画像として使用するピクチャを登録する
 * @type struct<picture>[]
 * 
 * @param ピクチャ_1
 * @parent ピクチャ登録
 * @desc picture_1 ピクチャ登録その1。どれを使ってもよい
 * @type struct<picture>[]
 *
 * @param ピクチャ_2
 * @parent ピクチャ登録
 * @desc picture_2 ピクチャ登録その2。どれを使ってもよい
 * @type struct<picture>[]
 * 
 * @param ピクチャ_3
 * @parent ピクチャ登録
 * @desc picture_3 ピクチャ登録その3。どれを使ってもよい
 * @type struct<picture>[]
 *
 * @param ピクチャ_4
 * @parent ピクチャ登録
 * @desc picture_4 ピクチャ登録その4。どれを使ってもよい
 * @type struct<picture>[]
 * 
 * @param ピクチャ_5
 * @parent ピクチャ登録
 * @desc picture_5 ピクチャ登録その5。どれを使ってもよい
 * @type struct<picture>[]
 *
 * @param ピクチャ_6
 * @parent ピクチャ登録
 * @desc picture_6 ピクチャ登録その6。どれを使ってもよい
 * @type struct<picture>[]
 * 
 * @param ピクチャ_7
 * @parent ピクチャ登録
 * @desc picture_7 ピクチャ登録その7。どれを使ってもよい
 * @type struct<picture>[]
 *
 * @param ピクチャ_8
 * @parent ピクチャ登録
 * @desc picture_8 ピクチャ登録その8。どれを使ってもよい
 * @type struct<picture>[]
 * 
 * @param ピクチャ_9
 * @parent ピクチャ登録
 * @desc picture_9 ピクチャ登録その9。どれを使ってもよい
 * @type struct<picture>[]
 * 
 * @param コモン登録
 * @desc commonMaking ビネットのコモンを登録する。パラパラアニメのような複数の画像で構成される
 * @type struct<vignetteCommon>[]
 *
 *
 *
 * @command ビネット表示
 * @desc showVignette ビネットを表示する
 * 
 * @arg タグ
 * @desc tag 何を書いてもいい欄
 *
 * @arg ビネット名
 * @desc vignetteName ビネットに付ける名前。アイリスエフェクト・ビネット消去で対象ビネットを指定するのに使う
 *
 * @arg 開始時に実行
 * @desc startingExe シーン開始時に自動的に実行する
 * @type boolean
 * 
 * @arg レイヤー
 * @desc layer 配置レイヤー。A はキャラの下、B はピクチャの下、Cはピクチャの上、Dはウインドウの上
 * @type select
 * @option レイヤーA1
 * @option レイヤーA2
 * @option レイヤーA3
 ...
 *
 * @arg …挿入位置
 * @desc insertPos 既存のビネットの上に表示するか、下に表示するか
 * @type select
 * @option 上
 * @option 下
 * @default 上
 *
 * @arg フェードイン時間
 * @desc fadeInTime ビネットの出現フェード時間。5 なら 5フレーム、1s なら 1秒
 *
 * @arg …フェード中ウェイト
 * @desc waitInFade フェードインが完了するまで待つ
 * @type boolean
 *
 * @arg ビネット持ち越し
 * @desc vignetteCarryOver マップ移動・戦闘移行時にビネットを消さずに持ち越す
 * @type boolean
 * 
 * @arg ピクチャ指定
 * @desc usePicture ピクチャを直接指定する。『Keke_CommonBase』で使うピクチャを登録すること(未使用素材にしないため)
 * @type file
 * @dir img/pictures
 * 
 * @arg ピクチャ使用
 * @desc usePicture ピクチャを使用する。プラグインパラメータでピクチャを登録→その名前を記入。\v[番号]で変数を使用可能
 * 
 * @arg └画面サイズに合わせる
 * @desc pictToScreen ピクチャの大きさを画面の大きさに合わせる
 * @type boolean
 * @default true
 * 
 * @arg 画像サイズ
 * @desc imageSizeRate ビネットとして表示する画像のサイズ。3 なら画面サイズの 3倍。空欄だと 3
 * 
 * @arg 内枠-フォーム
 * @desc inner-form ビネット内枠の形状
 * @type select
 * @option スクエア
 * @option 真円
 * @option 楕円
 ...
 *
 * @arg …フォーム設定
 * @desc formCfg 各フォームの詳細な設定
 * @type struct<formCfg>
 *
 * @arg 内枠-半径X
 * @desc inner-radiusX ビネット内枠の横半径。50 なら 画面横サイズの 50%
 * @default 40
 *
 * @arg 内枠-半径Y
 * @desc inner_radiusY ビネット内枠の縦半径。50 なら 画面縦サイズの 50%
 * @default 40
 *
 * @arg カラー赤
 * @desc colorRed ビネットの赤み。0〜255
 *
 * @arg カラー緑
 * @desc colorGreen ビネットの緑み。0〜255
 *
 * @arg カラー青
 * @desc colorBlue ビネットの青み。0〜255
 *
 * @arg ぼかし度
 * @desc blur ビネットのぼかしの強さ。0〜。値を増すほどぼやける
 *
 * @arg 不透明度
 * @desc opacity ビネットの不透明度。0〜255
 *
 * @arg ずらしX
 * @desc offsetX ビネットを横にずらす。5 なら 5ピクセル 右へ。js式として書ける。gw:画面横幅
 *
 * @arg ずらしY
 * @desc offsetY ビネットの縦にずらす。5 なら 5ピクセル 下へ。js式として書ける。gh:画面縦高
 *
 * @arg スケールX
 * @desc scaleX ビネットの横拡大率。1.5 なら 1.5倍
 *
 * @arg スケールY
 * @desc scaleY ビネットを縦拡大率。1.5 なら 1.5倍
 *
 * @arg 回転角
 * @desc angle ビネットを回転角。0〜360。90 で右に 90度、-90 で左に 90度
 *
 * @arg アニメーション
 * @desc animation アニメーションの設定をする。拡縮アニメと明滅アニメ
 * @type struct<animation>
 *
 * @arg 追尾設定
 * @desc chaseCfg 特定のキャラクターが常に中央に来るように追尾する。マップ時のみ
 * @type struct<chaseParam>
 *
 *
 *
 * @command アイリスエフェクト
 * @desc irisEffect ビネットを滑らかに形状変化させる
 *
 * @arg タグ
 * @desc tag 何を書いてもいい欄
 *
 * @arg 対象ビネット-名前
 * @desc subjectVignette-name 名前で対象ビネットを指定する。名前は表示時に設定したもの。, で区切って複数指定できる
 *
 * @arg 対象ビネット-ID
 * @desc subjectVignette-id IDで対象ビネットを指定する。IDは下に表示されてるのから順に 1〜。, で区切って複数指定できる
 *
 * @arg 開始時に実行
 * @desc startingExt シーン開始時に自動的に実行する
 * @type boolean
 *
 * @arg 動作時間
 * @desc runningTime アイリスエフェクトの所用時間。5 なら 5フレーム、1s なら 1秒
 * @default 20
 *
 * @arg …アニメ枚数
 * @desc animeSheetsNum アイリスエフェクトに使う画像の枚数。多いほど変化が滑らかだが、読み込みに時間がかかる。15以下推奨
 * @default 10
 *
 * @arg …動作中ウェイト
 * @desc waitRunning エフェクトが完了するまで待つ
 * @type boolean
 *
 * @arg 止めずに読み込み
 * @desc loadNonStop ゲームを止めずにデータを読み込む。止めないぶん少し読み込みが遅くなる
 * @type boolean
 *
 * @arg 実行後消去
 * @desc delAfterExe エフェクト実行後に自動的にビネットを消去する
 * @type boolean
 *
 * @arg イージング
 * @desc easing エフェクトに緩急をつける。イージング:遅→速→遅、イージングイン:遅→速、イージングアウト:速→遅
 * @type select
 * @option イージング無し
 * @option イージング
 * @option イージングイン
 ...
 *
 * @arg 内枠-フォーム
 * @desc inner-form ビネット内枠の形状を変更する
 * @type select
 * @option スクエア
 * @option 真円
 * @option 楕円
 ...
 *
 * @arg …フォーム設定
 * @desc formCfg 各形状に関する詳細な設定
 * @type struct<formCfg>
 *
 * @arg 内枠-半径X
 * @desc inner-radiusX ビネットの内枠横半径を変える。50 なら 画面横サイズの 50% にする
 *
 * @arg 内枠-半径Y
 * @desc inner-radiusY ビネットの内枠縦半径を変える。50 なら 画面縦サイズの 50% にする
 *
 * @arg カラー赤
 * @desc colorRed ビネットの赤みを変える。0〜255
 *
 * @arg カラー緑
 * @desc colorGreen ビネットの緑みを変える。0〜255。
 *
 * @arg カラー青
 * @desc colorBlue ビネットの青みを変える。0〜255
 *
 * @arg ぼかし度
 * @desc blur ビネットのぼかし度を変える。0〜。値を増すほどぼやける
 *
 * @arg 不透明度
 * @desc opacity ビネットの不透明度を変える。0〜255
 *
 * @arg ずらしX
 * @desc offsetX ビネットを横にずらす。5 なら 5ピクセル 右へ。js式として書ける。gw:画面横幅。即座に変わる
 *
 * @arg ずらしY
 * @desc offsetY ビネットを縦にずらす。5 なら 5ピクセル 下へ。js式として書ける。gh:画面縦高
 *
 *
 *
 * @command ビネット操作
 * @desc vignetteControl ビネットを移動・拡大・回転させる
 *
 * @arg タグ
 * @desc tag 何を書いてもいい欄
 *
 ...
 *
 * @arg 動作時間
 * @desc runningTime 操作の所用時間。5 なら 5フレーム、1s なら 1秒
 * @default 20
 *
 * @arg …動作中ウェイト
 * @desc waitRunning 操作が完了するまで待つ
 * @type boolean
 *
 * @arg 実行後消去
 * @desc delAfterExe 操作実行後に自動的にビネットを消去する
 * @type boolean
 * 
 * @arg イージング
 * @desc easing 操作に緩急をつける。イージング:遅→速→遅、イージングアウト:速→遅、イージングイン:遅→速
 * @type select
 * @option イージング無し
 * @option イージング
 * @option イージングアウト
 ...
 *
 * @arg スケールX
 * @desc scaleX ビネットを横に拡大する。1.5 なら 1.5倍 にする
 *
 * @arg スケールY
 * @desc scaleY ビネットを縦に拡大する。1.5 なら 1.5倍 にする
 *
 * @arg 回転角
 * @desc angle ビネットを回転させる。0〜360。90 で右に 90度、-90 で左に 90度 にする
 * 
 * @arg 不透明度
 * @desc opacity ビネットの不透明度を変える。0〜255
 *
 ...
 *
 * @command ビネット設定変更
 * @desc changeVignetteCfg ビネットの追尾とアニメーションの設定を変更する
 *
 * @arg タグ
 * @desc tag 何を書いてもいい欄
 *
 ...
 *
 * @arg アニメーション
 * @desc animation アニメーションの変更をする。拡縮アニメと明滅アニメ
 * @type struct<animation>
 *
 * @arg 追尾設定
 * @desc chaseCfg 特定のキャラクターが常に中央に来るように追尾する。マップ時のみ
 ...
 *
 * @command ビネット削除
 * @desc delVignette ビネットを削除する
 *
 * @arg タグ
 * @desc tag 何を書いてもいい欄
 *
 * @arg フェードアウト時間
 * @desc fadeOutTime ビネットの消去フェード時間。5 なら 5フレーム、1s なら 1秒
 * @default 
 *
 * @arg …フェード中ウェイト
 * @desc waitInFade フェードアウトが完了するまで待つ
 * @type boolean
 * @default 
 *
 * @arg 名前で削除
 * @desc delByName ビネット名でビネットを選択し削除する, で区切って複数指定可能
 *
 * @arg IDで削除
 * @desc delById IDでビネットを選択し削除する。IDは下に表示されてるのから順に 1〜。, で区切って複数指定できる
 *
 * @arg すべて削除
 * @desc delAll 全てのビネットを削除する
 * @type boolean
 * 
 * 
 * 
 * @command ビネットコモン
 * @desc vignetteCommon ビネットコモンを呼び出す
 * 
 * @arg タグ
 * @desc tag 何を書いてもいい欄
 *
 ...
 *
 * @arg 呼び出すコモン
 * @desc callCommon 呼び出すコモン。プラグインパラメータ→ビネットコモン で登録し、そのコモン名を記入
 *
 * @arg …取得ポイント
 * @desc getPoint ビネットコモンのどの地点の画像を取得するか。50 なら 50%地点
 * @default 0
 * 
 * @arg レイヤー
 * @desc layer 配置レイヤー。A はキャラの下、B はピクチャの下、Cはピクチャの上、Dはウインドウの上
 ...
 *
 * @arg スケールX
 * @desc scaleX ビネットを横に拡大する。1.5 なら 1.5倍
 *
 * @arg スケールY
 * @desc scaleY ビネットを縦に拡大する。1.5 なら 1.5倍
 *
 * @arg 回転角
 * @desc angle ビネットを回転させる。0〜360。90 で右に 90度、-90 で左に 90度
 *
 * @arg アニメーション
 * @desc animation アニメーションの設定をする。拡縮アニメと明滅アニメ
 * @type struct<animation>
 ...
 * 
 * @arg 開始時に実行
 * @desc startinExe シーン開始時に自動的に実行する
 * @type boolean
 *
 * @arg 呼び出すコモン
 * @desc callCommon 呼び出すコモンの名前。プラグインパラメータ→ビネットコモン で登録し、そのコモン名を記入
 *
 * @arg …開始ポイント
 * @desc getPoint ビネットコモンの開始点。50 なら 50%地点 から開始
 * @default 0
 *
 * @arg …終了ポイント
 * @desc endPoint ビネットコモンの終了点。50 なら 50%地点で終了
 * @default 100
 *
 * @arg 動作時間
 * @desc runningTime アイリスエフェクトの所用時間。5 なら 5フレーム、1s なら 1秒
 ...
 *
 * @arg …動作中ウェイト
 * @desc waitRunningエフェクトが完了するまで待つ
 * @type boolean
 *
 * @arg 止めずに読み込み
 * @desc loadNonStop ゲームを止めずにデータを読み込む。止めないぶん少し読み込みが遅くなる
...
//==================================================
 * @param コモン名
 * @desc commonName ビネットコモンの名前。コモン呼び出しに使う
 *
 * @param …アニメ枚数
 * @desc animeSheetsNum アイリスエフェクトに使う画像の枚数。多いほど変化が滑らかだが、読み込みに時間がかかる。15以下推奨
 * @default 10
 *
 * @param ピクチャ使用
 * @desc usePicture ピクチャを使用する。プラグインパラメータでピクチャを登録→その名前を記入
 * 
 * @arg 画像サイズ
 * @desc imageSizeRate ビネットとして表示する画像のサイズ。3 なら画面サイズの 3倍。空欄だと 3
 * 
 * @param イージング
 * @desc easing エフェクトに緩急をつける。イージング:遅→速→遅、イージングアウト:速→遅、イージングイン:遅→速
 * @type select
 * @option イージング無し
 * @option イージング
 * @option イージングアウト
 ...
 *
 * @param 内枠-半径Y
 * @desc inner-radiusY ビネット内枠の縦半径。50 なら 画面縦サイズの 50%
 * @default 0
 *
 * @param →内枠-半径X
 * @desc →inner-radiusX ビネットの内枠横半径を変える。50 なら 画面横サイズの 50% にする
...
//==================================================
 * @param スクエア-線丸み
 * @desc square-lineRound スクエア形の直線を丸くする。5 なら 5ピクセル分 丸まる。基本 0
 *
 * @param スクエア-角丸み
 * @desc square-CornerRound スクエア形の角を丸くする。5 なら 5ピクセル分 丸まる。基本 0
 *
 * @param 円形-扇型化
 * @desc circle-toFan 真円と楕円形を扇型にする。0, 90 なら 右から下、180, 270 なら左から上への扇型になる。基本 0, 0
 * @default 0, 0
 *
 * @param 楕円-回転角
 * @desc ellipse-angle 楕円形を回転させる。0〜360。90 で右に 90度、-90 で左に 90度。通常の回転角と違い、内枠部分のみ回転する。基本 0
 *
 * @param ダイヤ-線丸み
 * @desc diya-lineRound ダイヤ形の直線を丸くする。5 なら 5ピクセル 丸まる。基本 0
 *
 * @param ギザギザ-トゲ数X
 * @desc giza-thornNumX ギザギザ形の横方向のトゲの本数。1本〜。基本 1
 * @type number
 * @min 1
 *
 * @param ギザギザ-トゲ数Y
 * @desc giza-thornNumY ギザギザ形の縦方向のトゲの本数。1本〜。3 以上の奇数にすると電撃形になる。基本 1
 * @type number
 * @min 1
 *
 * @param ギザギザ-トゲ長さX
 * @desc giza-thornLengthX ギザギザ形の横方向のトゲの長さ。50 なら 50ピクセル。基本 50
 *
 * @param ギザギザ-トゲ長さY
 * @desc giza-thornLengthY ギザギザ形の縦方向のトゲの長さ。50 なら 50ピクセル。基本 50
 *
 * @param ギザギザ-トゲ長さ乱数
 * @desc giza-thornLengthRand ギザギザ形のトゲの長さのランダム幅。20 なら 80%〜120% の範囲で散らばる。基本 0
 *
 * @param 帯-線丸み
 * @desc belt-lineRound 帯形の直線を丸くする。5 なら 5ピクセル分 丸まる。基本 0
 *
 * @param フリー作図
 * @desc freeDesign 図形の構成点を入力。X は画面左端から 0〜100、Yは画面上端から 0〜100。x0 y50、x100 y50 は必ず通過
 * @type struct<freePos>[]
 */
...
//==================================================
 * @param X位置
 * @desc posX 点のX座標。画面横サイズに対する百分率。0 なら 画面左端、50 なら画面中央、100 なら画面右端
 *
 * @param Y位置
 * @desc posY 点のY位置。画面縦サイズに対する百分率。0 なら 画面上端、50 なら画面中央、100 なら画面下端
 */
 ...
 * @param 拡縮アニメ-動き
 * @parent 拡縮
 * @desc scaleAnime-move 拡縮アニメの動き。ノーマルは →、ターンは →←、ラウンドは →←←→ という動き
 * @type select
 * @option 拡縮無し
 * @option ノーマル
 * @option ターン
 ...
 * @param 拡縮アニメ-時間
 * @parent 拡縮
 * @desc scaleAnime-time 拡縮アニメの一周時間。5 なら 5フレーム、1s なら 1秒
 * @default 120
 *
 * @param 拡縮アニメ-大きさ
 * @parent 拡縮
 * @desc scaleAnime-vol 拡縮アニメの動きの大きさ。0.5 なら元の大きさの 0.5倍
 * @default 0.1
 *
 * @param 明滅
 * 
 * @param 明滅アニメ-動き
 * @parent 明滅
 * @desc flickAnime-move 明滅アニメの動き。ノーマルは →、ターンは →←、ラウンドは →←←→ という動き
 * @type select
 * @option 明滅無し
 * @option ノーマル
 * @option ターン
 ...
 * @param 明滅アニメ-時間
 * @parent 明滅
 * @desc flickAnime-time 明滅アニメの一周時間。5 なら 5フレーム、1s なら 1秒
 * @default 120
 *
 * @param 明滅アニメ-大きさ
 * @parent 明滅
 * @desc flickAnime-vol 明滅アニメの不透明度の変動量。50 なら 不透明度 50
 * @default -60
 *
 * @param 回転
 * 
 * @param 回転アニメ-動き
 * @parent 回転
 * @desc spinAnime-move 回転アニメの動き。ノーマルは →、ターンは →←、ラウンドは →←←→ という動き
 * @type select
 * @option 回転無し
 * @option ノーマル
 * @option ターン
 ...
 * @param 回転アニメ-時間
 * @parent 回転
 * @desc spinAnime-time 回転アニメの一周時間。5 なら 5フレーム、1s なら 1秒
 * @default 480
 *
 * @param 回転アニメ-大きさ
 * @parent 回転
 * @desc spinAnime-vol 回転アニメの回転量。50 なら右に 50度、-50 なら左に 50度
 * @default 360
 */
...
//==================================================
* @param 対象キャラ-名前
* @desc subjectChara-name ***→イベント検索、セルフ→イベント自身、プレイヤー→プレイヤー、全て→全キャラ。, で複数指定可
*
* @param 対象キャラ-ID
* @desc subjectChara-id 1〜→イベントID。0→イベント自身、-1→プレイヤー、-2→全キャラ。 , で複数指定可、~ でまとめて指定可
*
* @param 向きによる回転
* @desc spinByDire 追尾対象の向きに応じてビネットを回転させる。下向きが基準
* @type boolean
*
* @param …回転時間
* @desc spinTime 回転にかける時間。5 なら 5フレーム、1s なら 1秒
* @default 20
*
* @param 追尾解除
* @desc stopChase 追尾をやめる
* @type boolean
*/
...
//==================================================
 * @param ピクチャ名
 * @desc pictureName ピクチャに付ける名前。ピクチャ呼び出しに使う
 *
 * @param ファイル-タイトル1
 * @parent 画像登録
 * @desc file-title1 タイトル1の画像ファイル
 * @type file
 * @dir img/titles1
 *
 * @param ファイル-タイトル2
 * @parent 画像登録
 * @desc file-title2 タイトル2の画像ファイル
 * @type file
 * @dir img/titles2
 *
 * @param ファイル-遠景
 * @parent 画像登録
 * @desc file-parallax 遠景の画像ファイル
 * @type file
 * @dir img/parallaxes
 *
 * @param ファイル-戦闘背景1
 * @parent 画像登録
 * @desc file-battleBack1 戦闘背景1の画像ファイル
 * @type file
 * @dir img/battlebacks1
 *
 * @param ファイル-戦闘背景2
 * @parent 画像登録
 * @desc file-battleBack2 戦闘背景2の画像ファイル
 * @type file
 * @dir img/battlebacks2
 *
 * @param ファイル-ピクチャ
 * @parent 画像登録
 * @desc file-picture ピクチャの画像ファイル
 * @type file
 * @dir img/pictures
 */
...
(() => {
    ...
    // 開始時プラグインコマンド用のキャラスプライト保存変数
    let CharaSpritePlcm = null;
    // キャラスプライト初期化中フラグ
    let InCharaSpriteInit = null;
    // トループセットアップ中フラグ
    let InTroopSetup = false;
    ...
    function toBoolean(str) {
        ...
        if (str2 == "true" || str2 == "on" || Number(str2)) { return true; }
    ...
    //- ピクチャ登録
    let keke_pictureList = parameters["ピクチャ登録"] ? JSON.parse(parameters["ピクチャ登録"]).map(d => JSON.parse(d)) : [];

    for (let i = 1; i <= 9; i++) {
        const pictures =  parameters["ピクチャ_" + i] ? JSON.parse(parameters["ピクチャ_" + i]).map(d => JSON.parse(d)) : [];
        keke_pictureList = [...keke_pictureList, ...pictures]
    }
    ...
    PluginManager.registerCommand(pluginName, "ビネット表示", args => {
        // ハッシュの変数変換
        const args2 = convertVariableHash(args);
         // インタープリターを取得
        const preter = PlcmPreter;
        // ウェイトをセット
        const time = makeTime(args2["フェードイン時間"]) || 1;
        if (time <= 1 || toBoolean(args2["…フェード中ウェイト"])) { preter.wait(time); }
        args2.preter = preter;
        // ビネットの開始
        startVignette(args2);
    });
    ...
    PluginManager.registerCommand(pluginName, "アイリスエフェクト", args => {
        ...
        const preter = PlcmPreter;
        // 対象ビネットがなければリターン
        const vignettes = getTargetVignette(args2);
        if (!vignettes || !vignettes.length) { return; }
        // アイリスウェイトのセット
        const time = makeTime(args2["動作時間"]) || 1;
        if (time <= 1 || toBoolean(args2["…動作中ウェイト"])) { setIrisWait(preter); }
        args2.preter = preter;
        // アイリスエフェクトの開始
        startIrisEffect(args2);
    });
    ...
    PluginManager.registerCommand(pluginName, "ビネット操作", args => {
        ...
        // 操作ウェイトのセット
        const time = makeTime(args2["動作時間"]) || 1;
        if (time <= 1 || toBoolean(args2["…動作中ウェイト"])) { setControlWait(preter); }
        args2.preter = preter;
        // ビネット操作の開始
        startVignetteControl(args2);
    });
    ...
    PluginManager.registerCommand(pluginName, "ビネット削除", args => {
        ...
        const preter = PlcmPreter;
        // ウェイトをセット
        const time = makeTime(args2["フェードアウト時間"]) || 1;
        if (time <= 1 || toBoolean(args2["…フェード中ウェイト"])) { preter.wait(time); }
        // ビネットの削除
        eraseVignette(args2);
    });
    ...
    Sprite_Character.prototype.initialize = function(character) {
        _Sprite_Character_initialize.call(this, character);
         ...
         if (character._eventId && !$gameMap._pcStartedKeVnef && character._pageIndex >= 0) {
            setTimeout(setPcStarted, 0);
            CharaSpritePlcm = this;
            InCharaSpriteInit = true;
            runPluginCmdStarting(character.list(), [ /ビネット表示/, /アイリスエフェクト/, /ビネット操作/, /ビネット設定変更/, /ビネットコモン/, /アイリスコモン/], "開始時に実行");
            CharaSpritePlcm = null;
            InCharaSpriteInit = null;
            character._pcStartedKeVnef = true;
          }
    };

    function setPcStarted() {
        $gameMap._pcStartedKeVnef = true;
    };
    ...
    Game_Troop.prototype.setup = function(troopId) {
        ...
        // 開始時プラグインコマンドを実行
        runPluginCmdStarting(this.troop().pages[0].list, [/ビネット表示/, /アイリスエフェクト/, /ビネット操作/, /ビネット設定変更/, /ビネットコモン/, /アイリスコモン/], "開始時に実行");
        InTroopSetup = false;
    };


    //- シーンマップ/スプライトセットの形成(処理追加)
    const _Scene_Map_createSpriteset = Scene_Map.prototype.createSpriteset;
    Scene_Map.prototype.createSpriteset = function() {
        _Scene_Map_createSpriteset.apply(this);

        // ビネットレイヤーの作成
        createVignetteLayer(this);
    };

    //- シーンバトル/スプライトセットの形成(処理追加)
    const _Scene_Battle_createSpriteset = Scene_Battle.prototype.createSpriteset;
    Scene_Battle.prototype.createSpriteset = function() {
        _Scene_Battle_createSpriteset.apply(this);
    ...
    Scene_Map.prototype.terminate = function() {
        ...
        if ($gamePlayer._newMapId && $gamePlayer._newMapId != $gameMap.mapId()) {
            $gameMap._pcStartedKeVnef = false;
            // ビネットの自動クリア
            clearVignetteAuto("map");
        // それ以外
        } else {
    ...
    function getSceneType() {
        return $gameParty.inBattle() || InTroopSetup ? "battle" : "map";
    };
    ...
    function inSceneOpen() {
        if (InCharaSpriteInit) { return "map"; }
        if (InTroopSetup) { return "battle"; }
        return false;
    };
    ...
    //- ビネットレイヤーの作成2
    function createVignetteLayer2(scene) {
        const spriteset = scene._spriteset;
        if (!spriteset) { return; }
        // レイヤーC
        ...
        for (let i = 0; i < 3; i++) {
            ...
            //setTimeout(childLayer, 0, spriteset, sprite);
        };
    ...
    //- レイヤーチルド
    function childLayer(body, layer) {
        if (!body || !body.parent || !layer) { return; }
        body.addChild(layer);
    }
    ...
    function childVignette(sprite) {
        if (!sprite) { return; }
        const scene = SceneManager._scene;
        if (!scene._vignetteLayersKeA) { createVignetteLayer(SceneManager._scene._spriteset); }
        const vignette = sprite._vignetteKe;
        if (!vignette || !vignette.layer) { return; }
        const match = vignette.layer.match(/(レイヤー|)([ABCD])(\d+)/);
        if (!match) { return; }
        const layers = scene["_vignetteLayersKe" + match[2]];
        if (!layers) { return; }
        const layer = layers[Math.max(Number(match[3]) - 1, 0)];
        if (!layer) { return; }
    ...
    function isScreenLayer(freeHud) {
        return freeHud.layer.match(/[D]/i);
    };
    ...
    //- ゲームインタープリター/画像のロード(処理追加)
    const _Game_Interpreter_loadImages = Game_Interpreter.prototype.loadImages;
    Game_Interpreter.prototype.loadImages = function() {
        _Game_Interpreter_loadImages.apply(this);

        if (this.constructor.name == "Game_Interpreter" && !this._regKe) {
            // ピクチャの先行ロード
            preloadPicture(this._list);
        }
    };


    //- ピクチャの先行ロード
    function preloadPicture(list) {
        list = list.slice(0, 200);
        for (const cmd of list) {
            if (cmd.code == 357) {
                // 対象のプラグインコマンドか
                if (isCanPluginCmd(cmd, [/ビネット表示/, /ビネットコモン/])) {
                    const parameters = cmd.parameters;
                    // ビネット表示の場合
                    if (parameters[1] == "ビネット表示") {
                        const params = parameters[3];
                        if (params["ピクチャ使用"]) {
                            const pictName = params["ピクチャ使用"];
                            const url = getPictureUrl(pictName);
                            if (url) {
                                ImageManager.loadBitmapFromUrl(url);
                            }
                        }
                        if (params["ピクチャ指定"]) {
                            const url = params["ピクチャ指定"];
                            if (url) {
                                ImageManager.loadBitmapFromUrl("img/pictures/"+url+".png");
                            }
                        }
                    }
                    // ビネットコモンの場合
                    if (parameters[1] == "ビネットコモン") {
                        const params = parameters[3];
                        if (params["呼び出すコモン"]) {
                            const commonName = params["呼び出すコモン"];
                            const commonData = getCommonData(commonName)
                            if (commonData["ピクチャ使用"]) {
                                const pictName = commonData["ピクチャ使用"];
                                const url = getPictureUrl(pictName);
                                if (url) {
                                    ImageManager.loadBitmapFromUrl(url);
                                }
    ...
    //- ピクチャURLの取得
    function getPictureUrl(p, data) {
        if (p.url) { return p.url; }
        if (!data) {
            if (!p || !p.name) { return; }
            data = keke_pictureList.find(d => d["ピクチャ名"] == p.name);
            if (!data) { return; }
        }
        const files = getPictureFile(data);
        if (!files || !files.file) { return; }
        //return files;
        return "img/" + files.folder + Utils.encodeURI(files.file) + ".png";
    };
    ...
    function getPictureFile(data) {
        ...
        if (data["ファイル-ピクチャ"]) {
            ...
            folderName = "pictures/";
        } else if (data["ファイル-戦闘背景2"]) {
            fileName = data["ファイル-戦闘背景2"];
            folderName = "battleBacks2/";
        } else  if (data["ファイル-戦闘背景1"]) {
            fileName = data["ファイル-戦闘背景1"];
            folderName = "battleBacks1/";
        } else if (data["ファイル-遠景"]) {
            fileName = data["ファイル-遠景"];
            folderName = "parallaxes/";
        } else if (data["ファイル-タイトル2"]) {
            fileName = data["ファイル-タイトル2"];
            folderName = "titles2/";
        } else if (data["ファイル-タイトル1"]) {
            fileName = data["ファイル-タイトル1"];
            folderName = "titles1/";
        }
    ...
    function startVignette(args) {
        ...
        // 不透明度
        p.opacity = args["不透明度"] ? Number(args["不透明度"]) : 255;
        ...
        p.spinAnglePi = 0;
        // ずらし
        p.offsetX = getOffset(args["ずらしX"]) || 0;
        p.offsetY = getOffset(args["ずらしY"]) || 0;
        // ビネットアニメのセット
    ...
    function getOffset(str) {
        return Number(newFunc_screenW(str));
    };
    ...
    function convertVignetteParamBasic(p, args) {
        ...
        p.takeOver = args["ビネット持ち越し"] ? toBoolean(args["ビネット持ち越し"]) : p.takeOver;
        p.picture = getPictureCfg(args) || p.picture;
        p.pictToScreen = args["└画面サイズに合わせる"] || p.pictToScreen;
        p.imageSize = args["画像サイズ"] ? args["画像サイズ"] : p.imageSize;
    ...
    //- ピクチャ設定の取得
    function getPictureCfg(args) {
        if (args["ピクチャ指定"]) { return { url:"img/pictures/"+args["ピクチャ指定"]+".png" } }
        if (args["ピクチャ使用"]) { return { name:args["ピクチャ使用"] } }
        return null;
    };
    ...
    function convertVignetteParamForm(p, args) {
        // 半径
        p.radiusX = args["内枠-半径X"] ? (Number(args["内枠-半径X"]) / 100 || 0) : (p.radiusX || 0);
        p.radiusY = args["内枠-半径Y"] ? (Number(args["内枠-半径Y"]) / 100 || 0) : (p.radiusY || 0);
        // 通常パラム
        let params = ["red", "green", "blue", "blur"];
        let words = ["カラー赤", "カラー緑", "カラー青", "ぼかし度"];
        params.forEach((param, i) => {
            const word = words[i];
            p[param] = args[word] ? (Number(args[word]) || 0) : (p[param] || 0);
        });
        // フォームパラム
    ...
    function setVignetteAnime(vignette, args) {
        ...
        params.forEach((param, i) => {
            ...
            anime.start = param == "scale" ? 1 : param == "flick" ? 0 : 0;
            anime.speed = anime.pow / anime.timeMax;
            anime.pre = null;
            vignette[param] = anime;
        });
        // アニメーションしない時は効果量を戻す
        const scale = vignette.scale;
        if (!scale.moving || scale.moving == "拡縮無し" || !scale.timeMax || !scale.pow) {
            vignette.animeScale = 1;
        }
        const flick = vignette.flick;
        if (!flick.moving || flick.moving == "明滅無し" || !flick.timeMax || !flick.pow) {
            vignette.animeOpacity = 0;
        }
        const spin = vignette.spin;
        if (!spin.moving || spin.moving == "明滅無し" || !spin.timeMax || !spin.pow) {
    ...
    function setVignetteChase(vignette, args) {
        ...
        if (charas[0]) { chase.targetId = charaToId(charas[0]); }
        chase.spinByDire = params["向きによる回転"] ? toBoolean(params["向きによる回転"]) : chase.spinByDire;
    ...
    function createVignetteSprite(spriteset, sceneType, vignette, i) {
        ...
        // ピクチャが読み込み済みか
        if (!isLoadedPicture(vignette)) { return; }
        const gw = Graphics.width;
        const gh = Graphics.height;
        // ビネットビットマップの形成
        const r = vignette.commonName ? getVignetteCash(vignette.commonName, vignette.commonPoint) : createVignetteBitmap(vignette);
        const bitmap = r.bitmap;
        const toLate = r.toLate;
        // スプライト形成
        ...
        sprite.anchor.y = 0.5;
        // ピクチャを画面サイズに合わせる
        vignette.pictScaleX = r.pictScaleX;
        vignette.pictScaleY = r.pictScaleY;
        // カラートーン
        ...
        if (toLate) {
            ...
            setTimeout(showVignetteSprite, 0, sprite);
        }
    };


    //- ビネットスプライトの見える化
    function showVignetteSprite(sprite) {
        sprite.visible = true;
    };


    //- ピクチャが読み込み済みか
    function isLoadedPicture(vignette) {
        if (!vignette.picture) { return true; }
        const url = getPictureUrl(vignette.picture);
        if (!url) { return true; }
        const pictBitmap =  ImageManager.loadBitmapFromUrl(url);
        if (!pictBitmap) { return true; }
        return pictBitmap.isReady();
    };
    ...
    function createVignetteBitmap(vignette) {
        ...
        // ビットマップ形成
        const imageSize = vignette.imageSize || 3;
        let aWidth = 0;
        let aHeight = 0;
        let bitmap = null;
        // ピクチャ使用
        ...
        if (vignette.picture) {
            ...
            if (url) {
                const pictBitmap = ImageManager.loadBitmapFromUrl(url);
                aWidth = pictBitmap.width * imageSize;
                aHeight = pictBitmap.height * imageSize;
                bitmap = new Bitmap(aWidth, aHeight);
                bitmap._image = pictBitmap._image;
                bitmap.context.fillStyle = bitmap.context.createPattern(bitmap._image, 'repeat');
                // 画面サイズに合わせる
                if (vignette.pictToScreen) {
                    pictScaleX = Graphics.width / pictBitmap.width;
                    pictScaleY = Graphics.height / pictBitmap.height;
                }
            }
        }
        // ピクチャ使用しない場合
        if (!bitmap) {
            aWidth = gw * imageSize;
            aHeight = gh * imageSize;
            bitmap = new Bitmap(aWidth, aHeight);
            bitmap.context.fillStyle = color;
        }
        const context = bitmap.context;
        const iw = aWidth / imageSize;
        const ih = aHeight / imageSize;
        // ぼかし処理
        let blurH = 0;
        if (vignette.blur) {
            if (vignette.picture) {
                context.filter = `blur(${vignette.blur / 10}px)`;
            } else {
        ...
        const radiusX = vignette.radiusX * iw;
        const radiusY = vignette.radiusY * ih;
        const lengthX = radiusX * 2;
        const lengthY = radiusY * 2;
        const outW = (iw - lengthX) / 2;
        const outH = (ih - lengthY) / 2;
        const squareW = iw - outW * 2;
        const squareH = ih - outH * 2;
        ...
        let toLate = false;
        // 各フォームを描画
        const beltR = vignette.beltRound || 0;
        if (form == "スクエア" || !form) {
            ...
            designSquare(bitmap,  0,  0 - blurH - beltR * 0.4,  iw * 3,  ih + outH,  beltR);
            designSquare(bitmap,  0,  ih * 2 - outH - blurH + beltR * 0.4,  iw * 3,  ih + outH, beltR, 0,  "",  0,  true);
            toLate = true;
        } else if (form == "左右帯") {
            designSquare(bitmap,  0 - beltR * 0.4,  0 - blurH,  iw + outW,  ih * 3,   beltR);
            designSquare(bitmap,  iw * 2 - outW + beltR * 0.4,  0 - blurH,  iw + outW,  ih * 3,   beltR, 0,  "",  0,  true);
            toLate = true;
        } else if (form == "上帯") {
            designSquare(bitmap,  0,  0 - blurH - beltR * 0.4,  iw * 3,  ih + outH,  beltR);
            toLate = true;
        } else if (form == "下帯") {
            designSquare(bitmap,  0,  ih * 2 - outH - blurH + beltR * 0.4,  iw * 3,  ih + outH,  beltR);
            toLate = true;
        } else if (form == "左帯") {
            designSquare(bitmap,  0 - beltR * 0.4,  0 - blurH,   iw + outW, ih * 3,  beltR);
            toLate = true;
        } else if (form == "右帯") {
            designSquare(bitmap,  iw * 2 - outW + beltR * 0.4,  0 - blurH,  iw + outW,  ih * 3,  beltR);
            toLate = true;
        } else if (form == "フリー") {
        ...
        context.fill();
        return { bitmap:bitmap, toLate:toLate, pictScaleX:pictScaleX, pictScaleY:pictScaleY };
    };
    ...
    function spinVignetteByDire(vignette, init) {
        if (!vignette.chase || !vignette.chase.targetId || !vignette.chase.spinByDire) { return; }
        const chara = idToChara(vignette.chase.targetId);
    ...
    function updateVignetteSprite(spriteset, sceneType, vignette, i) {
        ...
        sprite.scale.x = vignette.scaleX * vignette.animeScale * vignette.pictScaleX * zoomRate;
        sprite.scale.y = vignette.scaleY * vignette.animeScale * vignette.pictScaleY * zoomRate;
        // 回転角
    ...
    function chaseVignetteSprite(sceneType, sprite, vignette, i) {
        ...
        const target = idToChara(vignette.chase.targetId);
        if (!target) { return; }
    ...
    function limitIrisEffectOne(args, handler, vignettes = [null]) {
        ...
        // 他がアイリス中orピクチャ読み込み中なら予約してリターン
        if (((inIrisEffect() || !SceneManager._scene._spriteset) && !(vignette && vignette.irisEffect)) || !isLoadedPicture(vignette)) {
            // アイリス予約のセット
    ...
    function updateIrisAppo(gm) {
        ...
        if (!isLoadedPicture(appo.vignette)) { return; }
    ...
    function startIrisEffect3(args, vignette) {
        ...
        // ずらしを適用
       vignette.offsetX = args["ずらしX"] ? getOffset(args["ずらしX"]) : vignette.offsetX;
       vignette.offsetY = args["ずらしY"] ? getOffset(args["ずらしY"]) : vignette.offsetY;
        // ビネットアニメのセット
    ...
    function startVignetteControl(args) {
        ...
        targets.forEach(vnt => {
            // 変動をセット
            const params = ["offsetX", "offsetY", "scaleX", "scaleY", "angle", "opacity"];
            const words = ["ずらしX", "ずらしY", "スケールX", "スケールY", "回転角", "不透明度"];
            params.forEach((param, i) => {
                const word = words[i];
                if (!args[word]) { return; }
                let tage = args[word];
                if (word.includes("ずらし")) { tage = getOffset(tage); }
                ctrl[param+"Drift"] = makeDrift(tage, vnt[param], ctrl.timeMax, ctrl.easing, word);
            });
            ctrl.delEndExe = toBoolean(args["実行後消去"]);
            // 操作をセット
    ...
    function changeVignetteConfig(args) {
        ...
        targets.forEach(vnt => {
            // ビネット持ち越しの変更
            vnt.takeOver = args["ビネット持ち越し"] ? toBoolean(args["ビネット持ち越し"]) : vnt.takeOver;
            // ビネットアニメのセット
    ...
    function convertIrisParamBasic(p, args, toRoute) {
        ...
        p.delEndExe = toBoolean(args["実行後消去"]);
    ...
    function convertIrisParamForm(p, args, vnt) {
        let radiusX = args["→内枠-半径X"] || args["内枠-半径X"] || 0;
        radiusX = radiusX ? (Number(radiusX) / 100).toString() : null;
        let radiusY = args["→内枠-半径Y"] || args["内枠-半径Y"] || 0;
        radiusY = radiusY ? (Number(radiusY) / 100).toString() : null;
        ...
        params.forEach((param, i) => {
            ...
            if (param.includes("radius")) {
                v = v ? (Number(v) / 100).toString() : null;
            }
            // 変動をセット
            if (v) {
                if (param == "circleToFan") {
    ...
    function loadIrisImage(packs) {
        ...
        if (!cashList[pack.index]) {
            // ビネットビットマップの形成
            const bitmap = createVignetteBitmap(iris).bitmap;
            // ウェイトをセット
            pack.loading = true;
    ...
    function delVignette(gm, ids, names, fadeTime) {
        ...
        vignettes.forEach((vnt, i) => {
            ...
            if (names && !names.some(n => vnt.name && vnt.name.includes(n))) { return; }
            vnt.delFlag = true;
            // フェードのセット
            setFade(vnt, fadeTime, -1);
            // ウェイトを消去
    ...
    //- ハッシュの変数置換
    function convertVariableHash(hash) {
        if (typeof hash != "object") { return; }
        const newHash = {};
        Object.keys(hash).forEach(k => {
            const p = hash[k];
            newHash[k] = p ? convertVariable(p) : p;
        });
        return newHash;
    };
    ...
    function copyHash(hash) {
        const copy = {};
        Object.keys(hash).forEach(k => {
            if (hash[k] && hash[k].constructor.name == "Object") {
                copy[k] = copyHash(hash[k]);
            } else if (hash[k] && hash[k].constructor.name == "Array") {
    ...
    function copyArray(array) {
        ...
        array.forEach((v, i) => {
            if (v && v.constructor.name == "Object") {
                copy[i] = copyHash(v);
            } else if (v && v.constructor.name == "Array") {
                copy[i] = copyArray(v);
            } else {
                copy[i] = v;
            }
    ...
    Game_InterpreterStartingKeVnef.prototype.canExecute = function(cmd) {
        result = false;
        if (cmd.code == 111 && cmd.parameters[0] != 12) { result = true; }
